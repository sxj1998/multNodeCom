...
void* thread_socket_server_sync(void* arg)
{
    ...
    socket_server_bind_socket(server_sockfd, &serv_addr);
    socket_server_start_listening(server_sockfd, MAX_CONNECTIONS);
    while (1)
    {
...
void* thread_socket_client_sync(void* arg)
{
    ...
    socket_client_address_init(&serv_addr, "127.0.0.1", 8888);
    connect_to_server(client_sockfd, &client_addr);
...
TEST_GROUP(cppUtest_socket_easy_test){
    void setup(){
...
TEST(cppUtest_socket_easy_test, check_usart_send_recv)
{
    while(1){
        sleep(1);
    }
